...
const NavLink = ({ link, active, mobile = false, t }) => (
  <Link
    ...
    className={mobile
      ? `min-w-[76px] flex flex-col items-center justify-center gap-1 px-2 py-2 text-xs border-t-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-yellow-400 ${active ? 'border-yellow-500 bg-yellow-500/10 text-yellow-200' : 'border-transparent text-gray-300 hover:text-yellow-200 hover:bg-gray-800/70'}`
      : `flex items-center gap-3 px-3 py-2 rounded-lg transition duration-150 border border-transparent focus-visible:outline focus-visible:outline-2 focus-visible:outline-yellow-400 ${active ? 'bg-yellow-500/10 text-yellow-200 border-yellow-700 shadow-[0_0_0_1px_rgba(234,179,8,0.3)]' : 'text-gray-300 hover:text-yellow-200 hover:bg-gray-800/60'}`}
  >
...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-midnight via-gray-950 to-black text-gray-100">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:rounded-lg focus:bg-yellow-300 focus:px-4 focus:py-2 focus:font-semibold focus:text-black focus:shadow-xl">
      ...
      <div className="flex">
        <aside className="w-64 bg-gray-950/75 border-r border-yellow-800/30 p-4 hidden md:block backdrop-blur-lg">
          ...
          <nav className="space-y-1" aria-label={t('nav.navigation')}>
            {sidebarLinks.map((link) => <NavLink key={link.to} link={link} active={location.pathname === link.to} t={t} />)}
          </nav>
        </aside>
        <main id="main-content" ref={mainRef} tabIndex={-1} aria-label={t('accessibility.main_content')} className="flex-1 min-w-0 p-4 pb-24 md:p-8 md:pb-8 space-y-6 relative overflow-hidden focus:outline-none">
      ...
      <nav className="md:hidden fixed inset-x-0 bottom-0 z-50 border-t border-yellow-800/40 bg-gray-950/95 backdrop-blur-xl shadow-[0_-8px_30px_rgba(0,0,0,0.45)]" aria-label={t('nav.mobile_navigation')} data-testid="mobile-navigation">
        <div className="flex overflow-x-auto overscroll-x-contain">
          {sidebarLinks.map((link) => <NavLink key={link.to} link={link} active={location.pathname === link.to} mobile t={t} />)}
        </div>
      </nav>
    </div>
  );
...
const GameRoute = ({ children }) => <ProtectedRoute><Layout>{children}</Layout></ProtectedRoute>;
...
const App = () => {
  ...
  useEffect(() => {
    if (user?.language && i18n.resolvedLanguage !== user.language) i18n.changeLanguage(user.language);
  }, [user?.language, i18n]);

  useEffect(() => {
    const unlockAudio = () => soundManager.unlock().catch(() => {});
  ...
  useEffect(() => {
    const handleClick = (event) => {
      ...
      if (target instanceof Element && target.closest('button')) {
        soundManager.unlock().then(() => soundManager.playSFX('click_ui')).catch(() => {});
      }
    };
  ...
  return (
    <Suspense fallback={<RouteFallback />}>
      <Routes>
        ...
        <Route path="/admin" element={<AdminRoute><Layout><AdminPanel /><AdminCityCreateCard /></Layout></AdminRoute>} />
