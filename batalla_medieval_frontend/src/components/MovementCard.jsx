...
const icons = {
  ...
  spy_out: 'spy',
  spy_in: 'spy',
  reinforce_out: 'shield',
  reinforce_in: 'handshake',
  transport_out: 'package',
  transport_in: 'inbox',
  return: 'return',
};

const movementLabels = { attack: 'Ataque', spy: 'Espionaje', reinforce: 'Refuerzo', transport: 'Transporte', return: 'Retorno' };
const statusLabels = { ongoing: 'En camino', resolved: 'Resuelto', completed: 'Completado', cancelled: 'Cancelado' };

const MovementCard = ({ movement }) => {
  const tone = colors[movement.category] || 'border-yellow-700/60 text-yellow-200 bg-yellow-900/20';
  const icon = icons[movement.category] || 'compass';
  const typeLabel = movementLabels[movement.movement_type] || movement.movement_type || 'Movimiento';
  const directionLabel = movement.category?.endsWith('_in') ? 'Entrante' : movement.category?.endsWith('_out') ? 'Saliente' : null;
  const targetLabel = movement.target_city_id ? `Ciudad #${movement.target_city_id}` : movement.target_oasis_id ? `Oasis #${movement.target_oasis_id}` : 'Destino no disponible';
    ...
    <article className={`card p-4 flex flex-col gap-3 relative overflow-hidden group ${tone} border`}>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <GameIcon name={icon} size={19} className="shrink-0" />
          <h3 className="text-lg truncate">{typeLabel}{directionLabel ? ` · ${directionLabel}` : ''}</h3>
        </div>
        <span className="badge shrink-0">{statusLabels[movement.status] || movement.status}</span>
      </div>
      <p className="text-sm text-gray-300 break-words">Ciudad #{movement.origin_city_id} → {targetLabel}</p>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-gray-400">
        <span className="tooltip" data-tip="Fecha y hora estimada">Llegada: {formatDate(movement.arrival_time)}</span>
