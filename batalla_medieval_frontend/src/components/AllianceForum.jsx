...
const AllianceForum = ({ alliance }) => {
  ...
  const handleCreateThread = async (event) => {
    ...
    try {
      ...
      setNewContent('');
      setView('list');
    } catch (error) { alert(error.response?.data?.detail || 'Error al crear hilo'); }
  };
  ...
  const handleReply = async (event) => {
    ...
    } catch (error) { alert(error.response?.data?.detail || 'Error al responder'); }
  };
  ...
  const handleModeration = async (changes) => {
    ...
    try {
      const response = await axiosClient.patch(`/forum/threads/${activeThread.id}/moderation`, changes);
  ...
  const ThreadFlags = ({ thread, compact = false }) => (
    <span className="inline-flex items-center gap-1.5" aria-label={[thread.is_pinned ? 'Fijado' : '', thread.is_locked ? 'Cerrado' : ''].filter(Boolean).join(', ')}>
      {thread.is_pinned && <span className="text-yellow-500"><GameIcon name="pin" size={compact ? 15 : 16} /></span>}
      {thread.is_locked && <span className="text-red-400"><GameIcon name="lock" size={compact ? 15 : 16} /></span>}
    </span>
  );
        ...
        <div className="card bg-black/40 border border-amber-900/30 p-6">
          ...
          {canModerate && (
            <div className="flex flex-wrap gap-2 mb-5" data-testid="forum-moderation-controls">
              <button type="button" className="btn btn-xs btn-outline" disabled={busyModeration} onClick={() => handleModeration({ is_pinned: !activeThread.is_pinned })} data-testid="forum-toggle-pin">{activeThread.is_pinned ? 'Desfijar' : 'Fijar'}</button>
              <button type="button" className="btn btn-xs btn-outline" disabled={busyModeration} onClick={() => handleModeration({ is_locked: !activeThread.is_locked })} data-testid="forum-toggle-lock">{activeThread.is_locked ? 'Reabrir' : 'Cerrar'}</button>
            </div>
          )}
