...
def test_process_all_queues_finishes(db_session, city):
    ...
    db_session.query(models.BuildingQueue).update({models.BuildingQueue.finish_time: datetime.utcnow() - timedelta(seconds=1)})
    troop_queue.finish_time = datetime.utcnow() - timedelta(seconds=1)
    db_session.commit()
