...
from app.services import production, research, troops, unit_catalog
...
def test_available_catalog_matches_research_and_training_payment(
    ...
    assert heavy["research_cost"] == pytest.approx(
        {"wood": 500.0, "clay": 400.0, "iron": 300.0}
    )
    ...
    db_session.refresh(city)
    for resource, amount in heavy["research_cost"].items():
        assert getattr(city, resource) == pytest.approx(
            before_research[resource] - amount
        )
    ...
    for resource, amount in expected_paid.items():
        assert getattr(city, resource) == pytest.approx(
            before_training[resource] - amount
        )
...
def test_cancel_training_refunds_exact_persisted_payment(db_session, city, monkeypatch):
    ...
    db_session.refresh(city)
    assert city.wood == pytest.approx(100.0 + 111.0 * troops.REFUND_FACTOR)
    assert city.clay == pytest.approx(100.0 + 222.0 * troops.REFUND_FACTOR)
    assert city.iron == pytest.approx(100.0 + 333.0 * troops.REFUND_FACTOR)
...
def test_due_training_cannot_be_cancelled(db_session, city, monkeypatch):
    ...
    queue = models.TroopQueue(
        ...
        paid_cost={"wood": 50.0, "clay": 30.0, "iron": 20.0},
    )
    db_session.add(queue)
    db_session.commit()
...
def test_training_completion_deletes_queue_before_side_effects(
    ...
    queue = models.TroopQueue(
        ...
        paid_cost={"wood": 150.0, "clay": 90.0, "iron": 60.0},
    )
    db_session.add(queue)
    db_session.commit()
