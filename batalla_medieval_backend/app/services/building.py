...
def queue_upgrade(db: Session, city: models.City, building_name: str) -> models.BuildingQueue:
    ...
    existing_queue = db.query(models.BuildingQueue).filter(models.BuildingQueue.city_id == city.id).count()
    ...
    already_queued = db.query(models.BuildingQueue).filter(
    ...
    building = db.query(models.Building).options(selectinload(models.Building.city)).filter(
    ...
    finish_time = utc_now() + timedelta(seconds=calculate_build_time(building_name, target_level))
...
def _run_completion_side_effects(db: Session, info: dict) -> None:
    ...
    try:
        quest_service.handle_event(
            ...
            user,
            "building_finished",
            {"building_type": info["building_type"], "level": info["target_level"]},
        )
...
def process_building_queues(db: Session) -> List[dict]:
    ...
    for queue_entry in finished_queues:
        ...
        if city is None:
            logger.error("building_queue_missing_city", extra={"queue_id": queue_entry.id, "city_id": queue_entry.city_id})
        ...
        if building is None:
            building = models.Building(city_id=queue_entry.city_id, name=queue_entry.building_type, level=0)
            db.add(building)
            db.flush()
        ...
        if building.level > previous_level:
            expansion_points_awarded = expansion_service.award_expansion_points_for_building(db, city, building.name)
        world_won = False
        if building.name == "world_wonder" and building.level >= maximum:
            world = db.query(models.World).filter(models.World.id == city.world_id).with_for_update().one_or_none()
    ...
    if finished_info:
        logger.info(
            ...
            extra={
                ...
                "expansion_points_awarded": sum(item["expansion_points_awarded"] for item in finished_info),
            },
        )
    return [
        {
...
def cancel_building_queue(db: Session, queue_id: int, user_id: int) -> bool:
    ...
    paid_cost = queue_entry.paid_cost or calculate_upgrade_cost(queue_entry.building_type, queue_entry.target_level)
    refund = {resource: float(amount) * REFUND_FACTOR for resource, amount in paid_cost.items()}
