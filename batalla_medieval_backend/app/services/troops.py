...
def get_unit_costs() -> Dict[str, Dict[str, float]]:
    return {unit_type: unit_catalog.get_unit(unit_type)["training_cost"] for unit_type in unit_catalog.UNIT_ORDER}


def get_training_times() -> Dict[str, int]:
    return {unit_type: int(unit_catalog.get_unit(unit_type)["training_time_seconds"]) for unit_type in unit_catalog.UNIT_ORDER}
...
def check_requirements(city: models.City, unit_type: str) -> None:
    ...
    missing = unit_catalog.first_missing_requirement(city, definition["training_requirements"])
    if missing:
...
def queue_training(db: Session, city: models.City, unit_type: str, quantity: int) -> models.TroopQueue:
    ...
    existing_queue = db.query(models.TroopQueue).filter(models.TroopQueue.city_id == city.id).count()
    ...
    total_cost = {resource: float(cost) * quantity for resource, cost in definition["training_cost"].items()}
    ...
    duration_seconds = float(definition["training_time_seconds"]) * quantity * training_time_multiplier
    finish_time = utc_now() + timedelta(seconds=duration_seconds)
    queue_entry = models.TroopQueue(city_id=city.id, troop_type=unit_type, amount=quantity, finish_time=finish_time, paid_cost={resource: float(amount) for resource, amount in total_cost.items()})
    db.add(queue_entry)
    db.commit()
    db.refresh(queue_entry)
    production.record_resource_gains(db, city, production_gains)
    logger.info("troop_training_queued", extra={"city_id": city.id, "unit_type": unit_type, "quantity": quantity, "paid_cost": queue_entry.paid_cost, "finish_time": finish_time.isoformat(), "upkeep_per_hour": float(definition.get("upkeep_per_hour", 0.0)) * quantity})
    return queue_entry
...
def _run_training_side_effects(db: Session, info: dict) -> None:
    ...
    if not user:
        return
    try:
        quest_service.handle_event(db, user, "troops_trained", {"unit_type": info["troop_type"], "amount": info["amount"]})
        achievement_service.update_achievement_progress(db, owner_id, "train_troops", world_id=world_id, increment=info["amount"])
        ranking.recalculate_player_and_alliance_scores(db, owner_id, world_id)
    except Exception:
        db.rollback()
        logger.exception("troop_training_side_effect_failed", extra={"city_id": info["city_id"], "unit_type": info["troop_type"], "amount": info["amount"]})


def process_troop_queues(db: Session) -> List[dict]:
    now = utc_now()
    finished_queues = db.query(models.TroopQueue).filter(models.TroopQueue.finish_time <= now).options(selectinload(models.TroopQueue.city)).order_by(models.TroopQueue.id.asc()).with_for_update(skip_locked=True).all()
    ...
    for queue_entry in finished_queues:
        ...
        if city is None:
            logger.error("troop_queue_missing_city", extra={"queue_id": queue_entry.id, "city_id": queue_entry.city_id})
            db.delete(queue_entry)
            continue
        troop = db.query(models.Troop).filter(models.Troop.city_id == queue_entry.city_id, models.Troop.unit_type == queue_entry.troop_type).with_for_update().one_or_none()
        if troop is None:
        ...
        internal_info.append({"city_id": queue_entry.city_id, "troop_type": queue_entry.troop_type, "amount": queue_entry.amount, "owner_id": city.owner_id, "world_id": city.world_id})
        db.delete(queue_entry)
    db.commit()
    for info in internal_info:
        _run_training_side_effects(db, info)
    logger.info("troop_training_completed", extra={"count": len(internal_info), "cities": [item["city_id"] for item in internal_info]})
    return [{"city_id": info["city_id"], "troop_type": info["troop_type"], "amount": info["amount"]} for info in internal_info]


def cancel_troop_queue(db: Session, queue_id: int, user_id: int) -> bool:
    queue_entry = db.query(models.TroopQueue).join(models.City).filter(models.TroopQueue.id == queue_id, models.City.owner_id == user_id).with_for_update().first()
    if not queue_entry:
        return False
    if _as_utc(queue_entry.finish_time) <= _as_utc(utc_now()):
        db.rollback()
        raise ValueError("Completed troop queue can no longer be cancelled")
    city, production_gains = production.lock_and_recalculate_resources(db, queue_entry.city_id)
    if queue_entry.paid_cost:
        paid_cost = {resource: float(amount) for resource, amount in queue_entry.paid_cost.items()}
    else:
        definition = unit_catalog.get_unit(queue_entry.troop_type)
        paid_cost = {resource: float(cost) * queue_entry.amount for resource, cost in definition["training_cost"].items()}
    refund = {resource: amount * REFUND_FACTOR for resource, amount in paid_cost.items()}
